	...
	)
{
	...
	const glm::vec3 modelPosition = glm::vec3(0, -0.5f, 0);
	std::unique_ptr<SceneNode> robot = builder.setTransform(modelPosition, modelOrientation, glm::vec3(0.1f))
		.addComponent<RenderableComponent>(robotModelId, shaderBasic1Id, Materials::green)
		.addComponent<RibbonComponent>(renderingEngine.ribbonManager.getRibbonBuffer(), 0.3f)
		.build();
	...
	root.add_child(std::move(
		builder.setTransform(glm::vec3(-2, 2, -2), noRotation, glm::vec3(0.1f))
		.addComponent<RenderableComponent>(cubeModelId, shaderBasic1Id, Materials::red)
		.addComponent<LightComponent>(light)
		.addComponent<AIComponent>(std::make_unique<RandomWanderBehavior>())
		.build()
	));
